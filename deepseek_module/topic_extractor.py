...
class TopicExtractor:
    ...
    async def _analyze_full_transcript(
        ...
        if is_coarse:
            ...
            target_len_line = "Целевая длительность топика 8–12 минут; МАКСИМУМ 15 минут (STRICT MUST). Если дольше — ОБЯЗАТЕЛЬНО разбей (STRICT MUST)."
            tail_line = "Равномерное покрытие всей лекции (MUST). Финальная треть без «длинного хвоста»: ни одна тема >15 минут (STRICT MUST)."
            last_window_line = "В последние 90 минут лекции выдели 7–12 подтем по 8–12 минут (STRICT MUST)."
            tail_check_line = "Для всех тем: длительность ≤15 минут; последний пункт ≤15 минут."
            last_window_check_line = "В последние 90 минут ≥7 подтем."
            gap_rule_line = "Если остались «дыры» (непокрытые содержательные интервалы вне пауз), разбей соседние темы так, чтобы соблюсти шаг 8–12 мин и лимит 15 мин."
...
ТРЕБОВАНИЯ:
    ...
    - {tail_line}
    - {last_window_line}
    - Перерыв: ЕСЛИ между соседними разделами есть явная пауза ≥15 минут, добавь РОВНО ОДИН пункт с названием ровно "Перерыв" (без указания длительности). Если пауз ≥30 минут несколько, выбери самую длинную. Иначе «Перерыв» не добавляй (STRICT MUST).
    - Перерывы/тишина/шум (MUST): внутри пауз НЕ выделяй темы (NEVER).
    - Если тема излишне большая (больше 15 минут) — разбей на меньшие темы (STRICT MUST).   
...
В конце проверь себя (MUST) и при необходимости переразметь:
    ...
    - Нет пересечений по времени с перерывом/тишиной/шумовыми вставками.
    - {tail_check_line}
    - Общее число тем в пределах [{min_topics}, {max_topics}].
    - {last_window_check_line}
    - {gap_rule_line}
    - ФИНАЛЬНЫЙ ОТРЕЗОК: последний топик заканчивается на последней содержательной реплике. Не допускай «длинного хвоста» в конце — если остаётся длинный участок, разбей его на несколько тем так, чтобы каждая была ≤15 минут, с предметными названиями.
    - Если нарушено ХОТЬ ОДНО правило (например, тема >15 минут или в последние 90 минут <7 подтем), ПЕРЕРАЗМЕТЬ список тем до полного выполнения всех правил. Возвращай только корректный итог без нарушений.
...
"""
    ...
    def _parse_structured_response(self, text: str, total_duration: float) -> dict[str, Any]:
        ...
        # 2) [HH:MM:SS - HH:MM:SS] (N мин) Название  — поддержка нового формата
        range_pattern = (
            r'\[(\d{1,2}):(\d{2})(?::(\d{2}))?\s*[-–—]\s*'
            r'(\d{1,2}):(\d{2})(?::(\d{2}))?\]\s*'
            r'(?:\((\d+)\s*мин\))?\s*'
