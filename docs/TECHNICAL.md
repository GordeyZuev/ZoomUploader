...
transcriptions/
└── <recording_id>/             # Папка транскрипции
...
```python
{
    ...
    "processed_audio_dir": str | None,
    "transcription_dir": str | None,
    "topic_timestamps": list[dict] | None,
...
```
transcription_dir/
├── words.txt          # Слова с точными временными метками
├── segments.txt       # Сегменты транскрипции
├── segments_auto.txt  # Автоматически сгенерированные (из words.txt) сегменты транскрипции
├── subtitles.srt      # Субтитры в формате SRT
└── subtitles.vtt      # Субтитры в формате VTT
```
